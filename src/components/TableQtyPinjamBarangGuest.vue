                ...
                <tbody>
                    <tr v-for="item in paginatedData" :key="item.id" class="hover:bg-slate-50 border-b border-slate-200 text-[#1E1E1E] text-sm">
                        ...
                        <td class="p-4 py-5 text-center text-[#0C8CE9] max-w-24">
                            <p class="">{{item.status_barang['nama']}}</p>
                        </td>
                    </tr>
                </tbody>
            </table>
      ...
      methods: {
          async fetchData() {
              try {
                  ...
                  const response = await axios.get("https://laravel-production-ea67.up.railway.app/api/user/barang", {
                      headers: {
